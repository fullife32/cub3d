...
/*   Updated: 2020/10/29 18:39:56 by eassouli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	key_front(t_a *a)
{
	if (a->mov.mov[FW] == OK)
	{
		...
		&& a->map.map[(int)a->plr.pos_y][(int)(a->plr.pos_x + a->dir.x * a->plr.move_spd)] != '1')
			a->plr.pos_x += a->dir.x * a->plr.move_spd;
		if (a->map.map[(int)(a->plr.pos_y + a->dir.y * a->plr.move_spd)][(int)a->plr.pos_x] != '1'
		&& a->map.map[(int)(a->plr.pos_y + a->dir.y * a->plr.move_spd)][(int)a->plr.pos_x] != '1')
...
void	key_side(t_a *a)
{
	if (a->mov.mov[L] == OK)
	{
		...
		&& a->map.map[(int)(a->plr.pos_y - a->dir.plane_y * a->plr.move_spd)][(int)a->plr.pos_x] != '1')
